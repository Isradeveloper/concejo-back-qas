...
export class EventCancellationMailUtil extends MailTemplate {
  constructor(participationRegister: ParticipationRegister) {
    ...
    if (!event) {
      throw new BadRequestException('Evento es requerido para el correo de cancelación');
    }

    const title = 'Cancelación de registro al evento de participación ciudadana';

    const bodyContent = `
    
    <span style="font-size: 14px;">
      Hola, <b>${MailTemplate.firstLetterCapitalize(user.firstName)} ${MailTemplate.firstLetterCapitalize(user.lastName || '')}</b> 👋 te confirmamos la cancelación de tu registro</b> al evento de participación ciudadana${participationRegister.organizationName ? `, en representación de ${MailTemplate.firstLetterCapitalize(participationRegister.organizationName)}` : ''}. A continuación relacionamos el detalle del evento cancelado: <br/></span>
    
    ${EventCancellationMailUtil.renderMeetingTemplate({
